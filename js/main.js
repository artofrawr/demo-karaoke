...
$(document).ready(function () {
  if (Detector.checkDependencies() == true) {
    ...
    if (!Detector.is_chrome || !Detector.is_firefox) {
      alert(
        "Dependency not met: This demo requires Chrome or Firefox. (detected: " +
          navigator.userAgent +
          ")"
      );
    }
...
function Karaoke() {
  ...
  this.soundManager = new SoundManager([
    "sounds/lucky_shorter_intro_music.mp3",
    "sounds/lucky_shorter_intro_vocals.mp3",
  ]);
...
Karaoke.prototype.render = function () {
  ...
  for (var tick = 0; tick <= 30; tick++) {
    var pitch = this.midiManager.getPitchFor(
