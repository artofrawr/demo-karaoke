    ...
    for (var s = 0; s < 5; s++) {
      ...
      for (c = 0; c < numCurvePoints; c++) {
        geometry.vertices.push(
          new THREE.Vector3(curvePoints[c], 0.5 * CURVE_HEIGHTS[s], 1)
        );
      }
  ...
  this.update = function (pitchRange, pitches, volumes) {
    ...
    for (var c = 0; c < curves_up.length; c++) {
      ...
      for (var i = 0; i < numCurvePoints; i++) {
        ...
        curve_geometry_up.vertices[i].y =
          CURVE_HEIGHTS[c] * 0.5 + p + volumes[i] * multiVol * easeMult * 40;
        curve_geometry_down.vertices[i].y =
          0 -
          CURVE_HEIGHTS[c] * 0.5 +
          p -
          volumes[i] * multiVol * easeMult * 40;
        count++;
      }
