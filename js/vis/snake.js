function Snake() {
  ...
  this.update = function (pitchRange, pitches, volume) {
    ...
    for (var i = numSnakePoints - 1; i >= 0; i--) {
      ...
      snake_5_geometry.vertices[i + numSnakePoints].y =
        0 - snake_5_height * 0.5 * v + p;

      snake_4_geometry.vertices[i].y = snake_4_height * 0.5 * v + p;
      snake_4_geometry.vertices[i + numSnakePoints].y =
        0 - snake_4_height * 0.5 * v + p;

      snake_3_geometry.vertices[i].y = snake_3_height * 0.5 * v + p;
      snake_3_geometry.vertices[i + numSnakePoints].y =
        0 - snake_3_height * 0.5 * v + p;

      snake_2_geometry.vertices[i].y = snake_2_height * 0.5 * v + p;
      snake_2_geometry.vertices[i + numSnakePoints].y =
        0 - snake_2_height * 0.5 * v + p;

      snake_1_geometry.vertices[i].y = snake_1_height * 0.5 * active * v + p;
      snake_1_geometry.vertices[i + numSnakePoints].y =
        0 - snake_1_height * 0.5 * active * v + p;
