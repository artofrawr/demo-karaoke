function Trail() {
  ...
  var render = function (pitchRange, volumes, songState, active) {
    ...
    if (Karaoke.soundPitch >= 0.9 && Karaoke.soundPitch <= 1.1) {
      ...
      pitchMultTop =
        Karaoke.soundPitch <= 0.9
          ? ((Karaoke.soundPitch - 0.5) / 0.5) * 0.3
          : ((1.5 - Karaoke.soundPitch) / 0.5) * 0.5;
      pitchMultBot =
        Karaoke.soundPitch >= 1.1
          ? ((1.5 - Karaoke.soundPitch) / 0.5) * 0.3
          : ((Karaoke.soundPitch - 0.5) / 0.5) * 0.5;
    }
    ...
    for (var i = numSnakePoints - 1; i >= 0; i--) {
      ...
      snake_1_geometry.vertices[i + numSnakePoints].y =
        p - newY * pMultiBot * actives[count];
